...
```kotlin
platformClient.order.getAllowedStateTransition(orderingChannel: orderingChannel, status: status).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.downloadLanesReport(body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.bulkStateTransistion(body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.bulkListing(pageSize: pageSize, pageNo: pageNo, startDate: startDate, endDate: endDate, status: status, bulkActionType: bulkActionType, searchKey: searchKey).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.jobDetails(batchId: batchId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.getFileByStatus(batchId: batchId, status: status, fileType: fileType, reportType: reportType).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.getManifestShipments(dpIds: dpIds, stores: stores, toDate: toDate, fromDate: fromDate, dpName: dpName, salesChannels: salesChannels, searchType: searchType, searchValue: searchValue, pageNo: pageNo, pageSize: pageSize).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.getManifests(status: status, startDate: startDate, endDate: endDate, searchType: searchType, storeId: storeId, searchValue: searchValue, dpIds: dpIds, pageNo: pageNo, pageSize: pageSize).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.processManifests(body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.getManifestDetails(manifestId: manifestId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.dispatchManifests(manifestId: manifestId, body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.uploadConsents(manifestId: manifestId, body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.getManifestfilters(view: view).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.trackShipment(shipmentId: shipmentId, awb: awb, pageNo: pageNo, pageSize: pageSize).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.updateShipmentTracking(body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.application("<APPLICATION_ID>").order.failedOrderLogs(pageNo: pageNo, pageSize: pageSize, searchType: searchType, searchValue: searchValue).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.failedOrderLogDetails(logId: logId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.getStateTransitionFlags(pageNo: pageNo, pageSize: pageSize).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.getStateTransitionFlagsById(flagId: flagId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.getStateTransitionFilters(pageNo: pageNo, pageSize: pageSize).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.getStateTransitionFiltersById(filterId: filterId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.getStateManagerTasks(pageNo: pageNo, pageSize: pageSize).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.getStateManagerTaskById(taskId: taskId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.application("<APPLICATION_ID>").order.getStateManagerStates(pageNo: pageNo, pageSize: pageSize).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.application("<APPLICATION_ID>").order.getShipmentBagReasons(shipmentId: shipmentId, lineNumber: lineNumber).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.getOrderById(orderId: orderId, myOrders: myOrders, allowInactive: allowInactive).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.application("<APPLICATION_ID>").order.getApplicationShipments(lane: lane, searchType: searchType, searchId: searchId, fromDate: fromDate, toDate: toDate, dpIds: dpIds, orderingCompanyId: orderingCompanyId, stores: stores, salesChannel: salesChannel, requestByExt: requestByExt, pageNo: pageNo, pageSize: pageSize, customerId: customerId, isPrioritySort: isPrioritySort, excludeLockedShipments: excludeLockedShipments).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.getBulkShipmentExcelFile(salesChannels: salesChannels, dpIds: dpIds, startDate: startDate, endDate: endDate, stores: stores, tags: tags, bagStatus: bagStatus, paymentMethods: paymentMethods, fileType: fileType, timeToDispatch: timeToDispatch, pageNo: pageNo, pageSize: pageSize).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.getAllowedTemplatesForBulk().safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.order.getTemplate(templateName: templateName).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
