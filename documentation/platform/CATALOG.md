...
```kotlin
client.catalog.upsertInventory(itemId: itemId, sellerIdentifier: sellerIdentifier, body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
client.catalog.upsertBulkInventory(body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
