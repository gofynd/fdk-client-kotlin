...
```kotlin
client.application("<APPLICATION_ID>").catalog.getAppInventory(itemIds: itemIds, storeIds: storeIds, brandIds: brandIds, sellerIdentifiers: sellerIdentifiers, timestamp: timestamp, pageSize: pageSize, pageId: pageId, isExtension: isExtension).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
| isExtension | Boolean? | no | For extensions support to retrieve inventory. |  
