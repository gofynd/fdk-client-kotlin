...
```kotlin
client.catalog.listAllProductTemplateExportDetails(status: status, fromDate: fromDate, toDate: toDate, q: q).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
client.catalog.getProductTemplateExportDetail(jobId: jobId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
client.catalog.updateProductExportJob(jobId: jobId, body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
client.catalog.listCategories(level: level, department: department, q: q, pageNo: pageNo, pageSize: pageSize).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
client.catalog.getInventoryExportJobDetail(jobId: jobId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
client.catalog.updateInventoryExportJob(jobId: jobId, body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
client.application("<APPLICATION_ID>").catalog.getAppicationProducts(q: q, f: f, c: c, filters: filters, isDependent: isDependent, sortOn: sortOn, pageId: pageId, pageSize: pageSize, pageNo: pageNo, pageType: pageType, itemIds: itemIds).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
client.application("<APPLICATION_ID>").catalog.getAppProducts(brandIds: brandIds, categoryIds: categoryIds, departmentIds: departmentIds, tags: tags, isDependent: isDependent, pageNo: pageNo, pageSize: pageSize, q: q).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
---
 ...
 | Properties | Type | Nullable | Description |
 | ---------- | ---- | -------- | ----------- |
 | items | ArrayList<[ProductTemplateCategory](#ProductTemplateCategory)>? |  yes  |  |
...
---
 ...
 | notificationEmails | ArrayList<String>? |  yes  | The list of notification email that should be notified on the completion of the job. |
...
---
 ...
 | ---------- | ---- | -------- | ----------- |
 | toDate | String? |  yes  | The modified on date till when the data needs to be exported. |
 | storeIds | ArrayList<Int>? |  yes  | The list of all the store selected. |
 | fromDate | String? |  yes  | The modified on date from which the data needs to be exported. |
 | brandIds | ArrayList<Int>? |  yes  | The list of all the brands selected. |
 | quantity | [InventoryExportQuantityFilter](#InventoryExportQuantityFilter)? |  yes  | The quantity range that needs to be exported. |
...
---
 ...
 | status | String? |  yes  | This tells you the current status of the export job. |
 | sellerId | Int |  no  | This ID of the company. |
 | filters | [InventoryExportCreateFilters](#InventoryExportCreateFilters) |  no  |  |
 | completedOn | String? |  yes  | This is the timestamp of the completion for this job. |
 | type | String? |  yes  | This is the file type of the export. |
 | cancelledOn | String? |  yes  | This is the timestamp of the cacellation for this job. |
 | createdBy | [UserDetail](#UserDetail)? |  yes  | This is the user detail of the user who cancelled the job. |
