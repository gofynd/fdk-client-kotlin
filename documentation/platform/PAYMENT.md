...
```kotlin
client.application("<APPLICATION_ID>").payment.edcDevice(terminalSerialNo: terminalSerialNo).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
client.application("<APPLICATION_ID>").payment.edcDevice(body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
