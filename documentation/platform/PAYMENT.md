...
```kotlin
client.application("<APPLICATION_ID>").payment.getPosPaymentModeRoutes().safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
client.application("<APPLICATION_ID>").payment.getPosPaymentModeRoutes(body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
