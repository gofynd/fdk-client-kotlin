...
```kotlin
client.application("<APPLICATION_ID>").payment.repaymentDetails(body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
client.application("<APPLICATION_ID>").payment.merchantOnBoarding(body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
