...
```kotlin
client.application("<APPLICATION_ID>").payment.edcDeviceList(pageNumber: pageNumber, isActive: isActive, storeId: storeId, deviceTag: deviceTag, pageSize: pageSize).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
