...
```kotlin
client.application("<APPLICATION_ID>").content.generateSEOTitle(type: type, body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
client.application("<APPLICATION_ID>").content.getBlogBySlug(slug: slug).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
