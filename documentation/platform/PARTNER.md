...
```kotlin
platformClient.partner.subscribeExtension(entity: entity, extensionId: extensionId, entityId: entityId, body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.partner.getPublicExtension(extensionId: extensionId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.partner.getExtensionById(extensionId: extensionId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.partner.deleteExtensionById(extensionId: extensionId, message: message, uninstallReasonType: uninstallReasonType).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.partner.getExtensionsSuggestions(pageSize: pageSize).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.partner.getPartnerInvites(requestStatus: requestStatus, pageSize: pageSize, pageNo: pageNo).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.partner.getPartnerRequestDetails(inviteId: inviteId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.partner.modifyPartnerRequest(inviteId: inviteId, body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.partner.setupProducts(requestId: requestId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.application("<APPLICATION_ID>").partner.getProxyPath(extensionId: extensionId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
platformClient.application("<APPLICATION_ID>").partner.getProxyPathAttachedPath(extensionId: extensionId, attachedPath: attachedPath).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
