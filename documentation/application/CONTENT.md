...
```kotlin
applicationClient.content.getSEOMarkupSchemas(pageType: pageType, active: active).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
applicationClient.content.getCustomObject(metaobjectId: metaobjectId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
applicationClient.content.getCustomFields(resource: resource, resourceId: resourceId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
