...
```kotlin
cart.getPromotionOffers(slug: slug, pageSize: pageSize).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
cart.getLadderOffers(slug: slug, promotionId: promotionId, pageSize: pageSize).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
