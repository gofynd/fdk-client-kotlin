...
```kotlin
cart.addItems(i: i, b: b, areaCode: areaCode, id: id, buyNow: buyNow, body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
cart.getPromotionOffers(slug: slug, pageSize: pageSize, promotionGroup: promotionGroup, storeId: storeId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
