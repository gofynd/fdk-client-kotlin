...
```kotlin
applicationClient.theme.getAppliedThemeV2().safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
applicationClient.theme.getThemeForPreviewV2(themeId: themeId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
