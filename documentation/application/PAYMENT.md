...
```kotlin
applicationClient.payment.getPaymentModeRoutes(amount: amount, cartId: cartId, checkoutMode: checkoutMode, refresh: refresh, orderId: orderId, cardReference: cardReference, userDetails: userDetails, displaySplit: displaySplit, advancePayment: advancePayment, shipmentId: shipmentId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
applicationClient.payment.walletLinkInitate(body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
applicationClient.payment.linkWallet(body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
applicationClient.payment.delinkWallet(body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
applicationClient.payment.cardDetails(cardInfo: cardInfo, aggregator: aggregator).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
applicationClient.payment.outstandingOrderDetails(aggregator: aggregator).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
applicationClient.payment.paidOrderDetails(aggregator: aggregator).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
applicationClient.payment.createPaymentOrder(body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
applicationClient.payment.deleteBeneficiaryDetails(beneficiaryId: beneficiaryId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
applicationClient.payment.getRefundOptions(configuration: configuration, productType: productType, amount: amount).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
applicationClient.payment.setRefundOptionforShipment(body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
applicationClient.payment.getSelectedRefundOption(shipmentId: shipmentId, orderId: orderId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
applicationClient.payment.getUserBeneficiariesDetailV2(orderId: orderId, shipmentId: shipmentId, mop: mop).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
applicationClient.payment.validateBeneficiaryAddress(body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
