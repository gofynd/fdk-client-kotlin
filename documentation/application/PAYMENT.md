...
```kotlin
payment.outstandingOrderDetails(merchantUserId: merchantUserId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
payment.paidOrderDetails(merchantUserId: merchantUserId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
