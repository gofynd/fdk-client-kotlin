...
```kotlin
payment.getPaymentLink(id: id).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
payment.getPaymentModeRoutesPaymentLink(id: id).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
payment.pollingPaymentLink(id: id).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
payment.redirectToAggregator(source: source).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
