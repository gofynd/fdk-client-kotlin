...
```kotlin
adminClient.communication.sendSellerCommunicationSynchronously(body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
adminClient.communication.sendSellerCommunicationAsynchronously(body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
