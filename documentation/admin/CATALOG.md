...
```kotlin
client.catalog.verifyProduct(itemId: itemId, body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
client.catalog.getProductVerificationDetails(itemId: itemId, id: id).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
catalog.createSizeGuide(body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
catalog.updateSizeGuide(id: id, body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
catalog.getSizeGuide(id: id).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
catalog.getAppProducts(brandIds: brandIds, categoryIds: categoryIds, departmentIds: departmentIds, tags: tags, itemIds: itemIds, pageNo: pageNo, pageSize: pageSize, q: q).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
catalog.listBulkImportJobs(jobType: jobType, search: search, stage: stage, startDate: startDate, endDate: endDate, pageNo: pageNo, pageSize: pageSize, total: total).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
catalog.createBulkImportJob(jobType: jobType, body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
catalog.updateBulkImportJob(jobType: jobType, jobId: jobId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
catalog.getBulkImportJob(jobType: jobType, jobId: jobId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
catalog.listBulkExportJobs(jobType: jobType, search: search, stage: stage, startDate: startDate, endDate: endDate, pageNo: pageNo, pageSize: pageSize, total: total).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
catalog.createBulkExportJob(jobType: jobType, body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
catalog.updateBulkExportJob(jobType: jobType, jobId: jobId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
catalog.getBulkExportJob(jobType: jobType, jobId: jobId).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
catalog.getImportValidationSchema(jobType: jobType).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
catalog.downloadImportFile(jobType: jobType).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
catalog.getAllHsnCodes(pageNo: pageNo, pageSize: pageSize, q: q, type: type).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
catalog.createHsnCodev2(body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
catalog.updateHsnCode(reportingHsn: reportingHsn, body: body).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
catalog.getSingleHSNCode(reportingHsn: reportingHsn).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
