...
```kotlin
panelClient.theme.getMarketplaceThemes(pageSize: pageSize, pageNo: pageNo).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
panelClient.theme.getMarketplaceThemeBySlug(slugName: slugName).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
...
```kotlin
panelClient.theme.getMarketplaceThemeVersions(slugName: slugName).safeAwait{ response, error->
    response?.let{
      // Use response
    } ->
    error?.let{
