...
class CartDataManagerClass(val config: ApplicationConfig, val unauthorizedAction: ((url: String, responseCode: Int) -> Unit)? = null) : BaseRepository() {
    ...
    fun getCart(id: String?=null, i: Boolean?=null, b: Boolean?=null, assignCardId: Int?=null, areaCode: String?=null, emptyCart: Boolean?=null): Deferred<Response<CartDetailResponse>>? {
        var fullUrl : String? = _relativeUrls["getCart"] 
        
        return cartApiList?.getCart(fullUrl    ,  id = id,    i = i,    b = b,    assignCardId = assignCardId,    areaCode = areaCode,    emptyCart = emptyCart)}
