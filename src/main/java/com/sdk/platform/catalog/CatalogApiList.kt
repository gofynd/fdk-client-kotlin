...
interface CatalogApiList {
    ...
    suspend fun getFollowedProducts(@Path("company_id") companyId: String, @Path("application_id") applicationId: String, @Path("user_id") userId: String, @Query("page_id") pageId: String?, @HeaderMap headers: Map<String, String>? = null)
    : Response<FollowedProducts>
    
    @POST ("/service/platform/catalog/v1.0/company/{company_id}/taxes/component-names")
    suspend fun createTaxComponentName(@Path("company_id") companyId: String,@Body body: CreateTaxComponentName, @HeaderMap headers: Map<String, String>? = null)
