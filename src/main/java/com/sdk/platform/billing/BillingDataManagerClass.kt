...
class BillingDataManagerClass(val config: PlatformConfig, val unauthorizedAction: ((url: String, responseCode: Int) -> Unit)? = null) : BaseRepository() {        
    ...
    suspend fun creditTransaction(uniqueId: Int, productSuite: String, type: String, pageSize: Int?=null, pageNo: Int?=null, sort: String?=null, query: HashMap<String,Any>?=null)
    ...
    suspend fun subscriptionPlanChange(productSuite: String?=null, uniqueId: Int?=null, platform: String?=null, planId: String?=null)
    : Response<PlanChangeDetails>? {
        
        return if (config.oauthClient.isAccessTokenValid()) {
            billingApiList?.subscriptionPlanChange(
        companyId = config.companyId, productSuite = productSuite, uniqueId = uniqueId, platform = platform, planId = planId )
        } else {
            null
        } 
    }
