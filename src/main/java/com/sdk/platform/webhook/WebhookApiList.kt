...
interface WebhookApiList {
    ...
    suspend fun manualRetryOfFailedEvent(@Path("company_id") companyId: String,@Body body: RetryEventRequest)
    : Response<RetrySuccessResponse>
    
    @POST ("/service/platform/webhook/v1.0/company/{company_id}/retry/events/counts")
    suspend fun getEventCounts(@Path("company_id") companyId: String,@Body body: RetryEventRequest)
