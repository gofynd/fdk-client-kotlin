...
class CatalogDataManagerClass(val config: PlatformConfig, val unauthorizedAction: ((url: String, responseCode: Int) -> Unit)? = null) : BaseRepository() {        
    ...
    suspend fun upsertInventory(itemId: String, sellerIdentifier: String,body: InventoryRequestSchemaV2)
    : Deferred<Response<SuccessResponse1>>? {
        
        return if (config.oauthClient.isAccessTokenValid()) {
            catalogApiList?.upsertInventory(
    ...
    suspend fun upsertBulkInventory(body: BulkInventoryRequestSchemaV2)
    : Deferred<Response<SuccessResponse1>>? {
        
        return if (config.oauthClient.isAccessTokenValid()) {
            catalogApiList?.upsertBulkInventory(
        companyId = config.companyId, body = body)
        } else {
            null
        } 
