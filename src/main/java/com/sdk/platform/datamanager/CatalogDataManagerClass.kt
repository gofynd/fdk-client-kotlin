...
class CatalogDataManagerClass(val config: PlatformConfig, val unauthorizedAction: ((url: String, responseCode: Int) -> Unit)? = null) : BaseRepository() {        
    ...
    suspend fun getProductAttributes(q: String?=null, department: ArrayList<Any>?=null, category: String?=null, filtered: Boolean?=null, pageNo: Int?=null, pageSize: Int?=null, sort: Int?=null)
    : Deferred<Response<ProductAttributesResponse>>? {
        
        return if (config.oauthClient.isAccessTokenValid()) {
            catalogApiList?.getProductAttributes(
        companyId = config.companyId, q = q, department = department, category = category, filtered = filtered, pageNo = pageNo, pageSize = pageSize, sort = sort )
        } else {
            null
        } 
    }
