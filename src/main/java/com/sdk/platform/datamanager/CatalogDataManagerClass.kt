...
class CatalogDataManagerClass(val config: PlatformConfig, val unauthorizedAction: ((url: String, responseCode: Int) -> Unit)? = null) : BaseRepository() {        
    ...
    suspend fun listProductTemplate(department: String, pageNo: Int?=null, pageSize: Int?=null)
    : Deferred<Response<TemplatesResponse>>? {
        
        return if (config.oauthClient.isAccessTokenValid()) {
            catalogApiList?.listProductTemplate(
        companyId = config.companyId, department = department, pageNo = pageNo, pageSize = pageSize )
        } else {
            null
        } 
    }
...
inner class ApplicationClient(val applicationId:String,val config: PlatformConfig){
    ...
    **/
    ...
    suspend fun getAppInventoryStatus(itemId: ArrayList<Int>?=null, storeIds: ArrayList<Int>?=null, brandIds: ArrayList<Int>?=null, sellerIdentifier: ArrayList<Int>?=null, timestamp: String?=null, pageSize: Int?=null, pageId: String?=null)
    : Deferred<Response<InventoryStockResponse>>? {
        return if (config.oauthClient.isAccessTokenValid()) {
                catalogApiList?.getAppInventoryStatus(companyId = config.companyId , applicationId = applicationId , itemId = itemId, storeIds = storeIds, brandIds = brandIds, sellerIdentifier = sellerIdentifier, timestamp = timestamp, pageSize = pageSize, pageId = pageId )
        } else {
            null
        }
    }
    ...
    **/
    ...
    suspend fun getApplicationCategoryListing(pageNo: Int?=null, pageSize: Int?=null, q: String?=null)
    : Deferred<Response<BrandListingResponse>>? {
        return if (config.oauthClient.isAccessTokenValid()) {
                catalogApiList?.getApplicationCategoryListing(companyId = config.companyId , applicationId = applicationId , pageNo = pageNo, pageSize = pageSize, q = q )
        } else {
            null
        }
    }
        ...
        paginator.setCallBack(object : PaginatorCallback<BrandListingResponse> {
            ...
            override suspend fun onNext(
                ...
                if (config.oauthClient.isAccessTokenValid()) {
                    ...
                    val pageNo = paginator.pageNo
                    val pageType = "number"
                    catalogApiList?.getApplicationCategoryListing(companyId = config.companyId , applicationId = applicationId , pageNo = pageNo, pageSize = pageSize, q = q)?.safeAwait{ response, error ->
                        response?.let {
                            val page = response.peekContent()?.page
                            paginator.setPaginator(hasNext=page?.hasNext?:false,pageNo=if (page?.hasNext == true) ((pageNo ?: 0) + 1) else pageNo)
                            onResponse.invoke(response,null)
