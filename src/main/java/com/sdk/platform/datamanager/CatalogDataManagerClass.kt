...
import com.sdk.platform.apis.catalog.*
...
class CatalogDataManagerClass(val config: PlatformConfig, val unauthorizedAction: ((url: String, responseCode: Int) -> Unit)? = null) : BaseRepository() {        
    ...
    : Deferred<Response<CrossSellingDataResponse>>? {
    ...
    suspend fun listDepartments(pageNo: Int?=null, pageSize: Int?=null, name: String?=null, search: String?=null, isActive: Boolean?=null, itemType: String?=null)
    : Deferred<Response<DepartmentListResponse>>? {
        
        return if (config.oauthClient.isAccessTokenValid()) {
            catalogApiList?.listDepartments(
        companyId = config.companyId, pageNo = pageNo, pageSize = pageSize, name = name, search = search, isActive = isActive, itemType = itemType )
        } else {
            null
        } 
    }
    ...
    suspend fun getDepartment(uid: String)
    : Deferred<Response<DepartmentUpdate>>? {
        
        return if (config.oauthClient.isAccessTokenValid()) {
            catalogApiList?.getDepartment(
...
inner class ApplicationClient(val applicationId:String,val config: PlatformConfig){
    ...
    suspend fun getAllSearchKeyword(isActive: Boolean?=null, q: String?=null)
    : Deferred<Response<GetSearchWordsResponse>>? {
        return if (config.oauthClient.isAccessTokenValid()) {
                catalogApiList?.getAllSearchKeyword(companyId = config.companyId , applicationId = applicationId , isActive = isActive, q = q )
        } else {
            null
        }
    }
    ...
    suspend fun updateAutocompleteKeyword(id: String,body: GetAutocompleteWordsData)
    : Deferred<Response<UpdateAutocompleteWordData>>? {
        return if (config.oauthClient.isAccessTokenValid()) {
    ...
    : Deferred<Response<GetAutocompleteWordsData>>? {
        return if (config.oauthClient.isAccessTokenValid()) {
    ...
    suspend fun listSearchRerankConfig(isActive: Boolean?=null, q: String?=null)
    : Deferred<Response<SearchRerankListing>>? {
        return if (config.oauthClient.isAccessTokenValid()) {
                catalogApiList?.listSearchRerankConfig(companyId = config.companyId , applicationId = applicationId , isActive = isActive, q = q )
        } else {
            null
        }
    }
    ...
    suspend fun createSearchRerankingConfig(body: CreateSearchReranking)
    : Deferred<Response<SearchRerankingModel>>? {
        return if (config.oauthClient.isAccessTokenValid()) {
                catalogApiList?.createSearchRerankingConfig(companyId = config.companyId , applicationId = applicationId , body = body)
        } else {
            null
        }
    }
    ...
    suspend fun updateSearchRerankConfig(id: String,body: CreateSearchReranking)
    : Deferred<Response<SearchRerankingModel>>? {
        return if (config.oauthClient.isAccessTokenValid()) {
                catalogApiList?.updateSearchRerankConfig(companyId = config.companyId , applicationId = applicationId , id = id, body = body)
        } else {
            null
        }
    }
    ...
    suspend fun getSearchRerankingConfig(id: String)
    : Deferred<Response<SearchRerankingModel>>? {
        return if (config.oauthClient.isAccessTokenValid()) {
                catalogApiList?.getSearchRerankingConfig(companyId = config.companyId , applicationId = applicationId , id = id )
        } else {
            null
        }
    }
    ...
    suspend fun deleteSearchRerankConfig(id: String)
    : Deferred<Response<DeleteRerankResponse>>? {
        return if (config.oauthClient.isAccessTokenValid()) {
                catalogApiList?.deleteSearchRerankConfig(companyId = config.companyId , applicationId = applicationId , id = id )
        } else {
            null
        }
    }
    ...
    : Deferred<Response<CollectionUpdateResponse>>? {
        return if (config.oauthClient.isAccessTokenValid()) {
    ...
    suspend fun getCollectionItems(id: String, f: String?=null, sortOn: String?=null, pageId: Int?=null, pageSize: Int?=null, xCurrencyCode: String?=null)
    ...
    **/
    ...
    suspend fun getAppProducts(brandIds: ArrayList<Int>?=null, categoryIds: ArrayList<Int>?=null, departmentIds: ArrayList<Int>?=null, tags: ArrayList<String>?=null, isDependent: Boolean?=null, itemIds: ArrayList<Int>?=null, pageNo: Int?=null, pageSize: Int?=null, q: String?=null)
    : Deferred<Response<ProductListingResponse>>? {
        return if (config.oauthClient.isAccessTokenValid()) {
                catalogApiList?.getAppProducts(companyId = config.companyId , applicationId = applicationId , brandIds = brandIds, categoryIds = categoryIds, departmentIds = departmentIds, tags = tags, isDependent = isDependent, itemIds = itemIds, pageNo = pageNo, pageSize = pageSize, q = q )
        } else {
            null
        }
    }
