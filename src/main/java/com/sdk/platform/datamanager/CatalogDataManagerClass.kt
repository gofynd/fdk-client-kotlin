...
inner class ApplicationClient(val applicationId:String,val config: PlatformConfig){
    ...
    **/
    ...
    suspend fun updateAppLocation(brandUid: String, storeUid: String,body: ApplicationStoreJson)
    : Deferred<Response<SuccessResponse>>? {
        return if (config.oauthClient.isAccessTokenValid()) {
                catalogApiList?.updateAppLocation(companyId = config.companyId , applicationId = applicationId , brandUid = brandUid, storeUid = storeUid, body = body)
        } else {
            null
        }
    }
